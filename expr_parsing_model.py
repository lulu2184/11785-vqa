...
from expr_parsing_attention import ExprParsingAttention
from language_model import WordEmbedding, QuestionEmbedding
from nonlinear import NonLinearLayer


class ExprParsingAttentionModel(nn.Module):
    def __init__(self, w_emb, q_emb, v_att, q_net, v_net, classifier):
        super(ExprParsingAttentionModel, self).__init__()
        self.w_emb = w_emb
        self.q_emb = q_emb
        self.v_att = v_att
        self.q_net = q_net
...
def build_expr_parsing_attention_model(dataset, num_hid):
    w_emb = WordEmbedding(dataset.dictionary.ntoken, 300,
                          'data/glove6b_init_300d.npy')
    q_emb = QuestionEmbedding(300, num_hid, 1, False)
    v_att = ExprParsingAttention(dataset.v_dim, q_emb.hidden_num, num_hid)
    ...
    return ExprParsingAttentionModel(
